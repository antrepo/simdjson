...
public:
   ...
   */
  element_result<element> root() const noexcept;
...
public:
   ...
   *         - UNEXPECTED_TYPE error if the JSON element is not a string
   */
  element_result<const std::string_view> as_string() const noexcept;
   ...
   */
  element_result<uint64_t> as_uint64() const noexcept;
   ...
   */
  element_result<int64_t> as_int64() const noexcept;
   ...
   */
  element_result<document::object> as_array() const noexcept;
  ...
  /**
   * When this is used as an iterator, this moves to the next element.
   *
   * Part of the std::iterator interface.
   */
  const document::element operator++() const noexcept;
  /**
   * Tells whether this element is lexically before or after the other element.
   *
   * **Assumes** they are in the same document.
   *
   * Part of the std::iterator interface.
   */
  bool operator<(const document::element& other) const noexcept;
...
public:
   ...
   */
  const document::element begin() const noexcept;
  /**
   * One past the last array element.
   *
   * Part of the std::iterable interface.
   */
  const document::element end() const noexcept;
...
public:
   ...
   * Part of the std::iterable interface.
   */
  const document::key_value_pair begin() const noexcept;
  /**
   * One past the last key/value pair.
   *
   * Part of the std::iterable interface.
   */
  const document::key_value_pair end() const noexcept;
 ...
 */
class document::key_value_pair : document::tape_ref {
public:
  /**
   * The key.
   */
  ...
  /**
   * The key, as a null-terminated C string.
   */
  const char *key_c_str() const noexcept;
  /**
   * The value.
   */
  const document::element value() const noexcept;
  /**
  ...
  /**
   * Check if this is before the other element.
   *
   * Part of the std::iterator interface.
   */
  bool operator<(const document::key_value_pair& other) const noexcept;

private:
  key_value_pair(const document &_doc, size_t _json_index) noexcept;
...
private:
  element_result(error_code _error, T _value) noexcept;
...
public:
  ...
  */
  ...
  // JSON).
  WARN_UNUSED bool print_json(std::ostream &os) const noexcept;
  WARN_UNUSED bool dump_raw_tape(std::ostream &os) const noexcept;
